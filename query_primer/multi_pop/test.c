...
static char* test_1() {
	static char *lines[200001];
	static char chars[100000][5];
	lines[0] = "100000 100000";
	for (size_t i = 0; i < 100000; i++) {
		snprintf(chars[i], sizeof(chars[i]), "%zu", i % 10000);
		lines[i + 1] = chars[i];
		lines[i + 100001] = i % 11111 ? "pop" : "show";
	}
	static char *expected[500050];
	size_t k = 0;
	for (size_t i = 0; i < 10; i++) {
		for (size_t j = i * 11110; j < 100000; j++) {
			expected[k++] = chars[j];
		}
	}
	return test(200001, (const char**)lines, 500050, (const char**)expected);
}
