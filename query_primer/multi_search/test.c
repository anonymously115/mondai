...
static char* test_1() {
	char *in[200001] = { "100000 100000" };
	char A[100000][8] = { };
	char K[100000][8] = { };
	char *out[100000] = { };
	for (size_t i = 0; i < 100000; i++) {
		snprintf(A[i], sizeof(A[i]), "%zu", 10 * (i + 1));
		in[i + 1] = A[i];
		snprintf(K[i], sizeof(K[i]), "%zu", i + 900001);
		in[i + 100001] = K[i];
		out[i] = (i + 1) % 10 ? "NO" : "YES";
	}
	return test(200001, (const char**) in, 100000, (const char**) out);
}
